...
class ChartingState extends MusicBeatState
{
	...
	var bgF = new FlxSprite(-80).loadGraphic(Paths.image('menuBGBlue'));
	...
	**/
	...
	override function create()
	{
		...
		bgF.scrollFactor.x = 0;
		bgF.scrollFactor.y = 0;
		bgF.setGraphicSize(Std.int(bgF.width * 1.1));
		bgF.updateHitbox();
		bgF.alpha = 0;
		bgF.screenCenter();
		...
		leftIcon.setPosition(0, -130);
		rightIcon.setPosition(gridBG.width / 2, -130);

		bpmTxt = new FlxText(1000, 50, 2000, "", 16);
		bpmTxt.setFormat("Pixel Arial 11 Bold", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override function update(elapsed:Float)
	{
		...
		bgF.alpha = FlxMath.lerp(0, bgF.alpha, 0.95);
		...
		{
			openSubState(new ChartingOptions(curSection, _song, vocals));
		}
	...
	public static function playChart(sec:Int, sng:SwagSong, snd:FlxSound):Void
		{
			lastSection = sec;
